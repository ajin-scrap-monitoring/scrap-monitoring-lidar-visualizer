"""Strict parser for Observation version 1 JSON Lines records."""
...
class ContractParser:
    ...
    def __init__(self, contract_root: Path | None = None) -> None:
        ...
        self._validators = {
            "load_model_stream_header": self._load_validator(
                root / "header.schema.json"
            ),
            "load_model_observation": self._load_validator(
    ...
    def parse_line(self, raw_line: bytes) -> ParsedRecord:
        ...
        if record_type == "load_model_stream_header":
            model: Header | Observation = self._header(cast(dict[str, Any], value))
        else:
            model = self._observation(cast(dict[str, Any], value))
        return ParsedRecord(raw_line=raw_line, value=model)

    def _header(self, value: dict[str, Any]) -> Header:
