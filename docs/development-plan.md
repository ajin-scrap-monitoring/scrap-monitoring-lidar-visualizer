...
| 구분 | 내용 |
| --- | --- |
| 산출물 | 표면, 닫힌 적재 체적과 바닥의 삼각형 mesh, 외벽, sensor, 투입구, 카메라와 overlay |
| 수치 검증 | 비대칭 Y-major 격자, 비영점 바닥 높이, concave 경계, 경계를 교차하는 cell, 퇴화 입력 |
| 결정론 검증 | 동일 입력의 vertex, face와 출력 순서 일치 |
| 장면 검증 | 직교 투영 사선 및 상면 카메라, 고정 높이 색상과 범례, 중립색 격자, sensor 회전축, filling 투입구, collecting의 비활성 투입구, 필수 overlay |
...
| 구분 | 내용 |
| --- | --- |
| 산출물 | Live CLI, preview 경로, camera 및 상면 프레임 저장소와 브라우저 화면 |
